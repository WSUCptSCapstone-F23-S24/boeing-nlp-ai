...
def test_extract_text_from_pdf_with_valid_path():
    ...
    expected_text = "Test for extracting text from PDF!"
    
    # function to test
    extracted_text = app.main.extract_text_from_pdf(test_pdf_path)
...
def test_extract_text_from_pdf_with_invalid_path():
    extracted_text = app.main.extract_text_from_pdf("does/not/exist/pdf.pdf")
...
def test_process_large_document_with_pdf_file():
    ...
    formatted_sentences, tokens, entities, doc = app.main.process_large_document(document_path)
...
def test_generate_summary():
    ...
    summary = app.main.generate_summary(doc, ratio=0.5)
